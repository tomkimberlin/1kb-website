...
import argparse, json, socket, ssl, time, select
from pathlib import Path
from collections import Counter
import h2.connection,h2.config,h2.events

p=argparse.ArgumentParser();p.add_argument('--host',default='tomkimberlin.com');p.add_argument('--ip');p.add_argument('--port',type=int,default=443);p.add_argument('--out',required=True);p.add_argument('--minimal',action='store_true');p.add_argument('--single',action='store_true');p.add_argument('--tls12',action='store_true');p.add_argument('--source-port',type=int,default=0);p.add_argument('--ca');p.add_argument('--insecure',action='store_true');a=p.parse_args()
...
for connection_index in range(2):
 ...
 def flush():
  while outgoing.pending:
   b=outgoing.read();sock.sendall(b);wire['sent']+=len(b);blobs['sent'].extend(b)
 def receive():
  b=sock.recv(65536)
 ...
 for stream_id in ([1] if a.single else [1,3]):
  ...
  while not done:
   ...
   for e in conn.receive_data(b):
    ...
    elif isinstance(e,h2.events.DataReceived) and e.stream_id==stream_id:body.extend(e.data);conn.acknowledge_received_data(e.flow_controlled_length,e.stream_id)
    elif isinstance(e,h2.events.StreamEnded) and e.stream_id==stream_id:done=True
   b=conn.data_to_send()
   if b:outgoing_plain.extend(b);send(b)
  requests.append({'stream':stream_id,'body_bytes':len(body),'headers':response,'tls_bytes_increment':{k:wire[k]-before[k] for k in wire},'elapsed_ms':round((time.monotonic()-start)*1000,2)})
  snapshots.append({'phase':f'response_{stream_id}_complete',**wire,'time_ns':time.time_ns()})
  if stream_id==1 and bytes(body)!=(Path(__file__).resolve().parents[1]/'public/index.html.br').read_bytes():raise AssertionError('Body mismatch')
 # Process any immediately available post-handshake messages without blocking long.
 while select.select([sock],[],[],0.08)[0]:
  receive()
  try:
   b=tls.read(65536)
