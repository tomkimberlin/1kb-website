...
The same idea used for the HTML also helped the certificates: tune the encoder offline, then serve the resulting standard Brotli bytes. A separate optimizer found another nine bytes on the main certificate, eight on `www`, and two on the alternate domain. It receives only public certificate chains, with no network access or private-key mount.
