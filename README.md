...
| Brotli response body | 319 B | **315 B** |
| Gzip response body | 471 B | 468 B |
| Deflate response body | 459 B | 456 B |
| Raw HTML | 746 B | 743 B |

The local build preserves the page's content and appearance. It has not been deployed; the published measurements remain the September 22 snapshot.
...
[Delivery settings](TRANSPORT.md), [gallery comparison](COMPARISON.md), [build sizes](build-report.json), [local checks](measurements/page-20260926-local.json), [published browser measurements](measurements/page-20260922.json).
...
For browser comparisons, install the optional test tools and save the original page before editing:

```sh
npm install --no-save --package-lock=false playwright
npx playwright install chromium webkit
mkdir -p optimization
git show da7b5f5:index.html > optimization/baseline.html
